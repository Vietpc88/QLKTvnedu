...
import { saveToFirebase, loadFromFirebase, updateFieldInFirebase } from './firebase';
...
export const saveToGas = async (gasUrl: string, payload: any, action: 'sync' | 'append' | 'updateStatus' | 'updateSpeakingScore' | 'delete' = 'sync') => {
  ...
  if (storageType === 'firebase') {
    if (action === 'updateSpeakingScore' && payload.updates) {
      // Handle specific update for speaking score if needed, 
      // but for simplicity we can just sync the whole thing for now 
      // or implement partial updates.
      // Since we want a smooth transition, let's just save the payload.
