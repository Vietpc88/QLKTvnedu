    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center overflow-hidden bg-slate-950 text-white font-sans">
      ...
      <div className="relative z-10 w-full max-w-lg p-6 flex flex-col items-center">
        ...
        <div className="w-full bg-slate-900/50 backdrop-blur-2xl border border-white/10 rounded-[2.5rem] p-8 shadow-2xl animate-in fade-in slide-in-from-bottom-8 duration-1000">
          {/* Tab Selection */}
          <div className="flex bg-slate-950/50 p-1.5 rounded-2xl mb-8 border border-white/5 flex-wrap gap-1">
            <button
            ...
            <button
              ...
              className={cn(
                "flex-1 py-3 px-2 md:px-4 rounded-xl text-xs md:text-sm font-bold transition-all flex items-center justify-center gap-2",
                loginTab === 'speaking_teacher' 
                  ? "bg-indigo-600 text-white shadow-lg shadow-indigo-600/20" 
                  : "text-slate-500 hover:text-slate-300"
              )}
            >
              <User size={16} /> ĐIỂM NÓI
            </button>
            <button
          ...
          <div className="mt-8 text-center">
            <p className="text-[11px] text-slate-500 italic leading-relaxed">
              ...
              {loginTab === 'speaking_teacher' && 'Dành cho GV Tiếng Anh đăng nhập để nhập điểm nói theo họ tên.'}
            </p>
          </div>
        </div>
